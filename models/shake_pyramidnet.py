...
class ShakeBasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_zero = torch.zeros(h0.size()[0], h.size()[1] - h0.size()[1], h0.size()[2:4][0], h0.size()[2:4][1], device='cuda', dtype=torch.float32) 
...
def ShakePyramidNet110_270(num_classes=10):
    return ShakePyramidNet(depth=110, alpha=270, num_classes=num_classes)

def ShakePyramidNet272_200(num_classes=10):
    return ShakePyramidNet(depth=272, alpha=200, num_classes=num_classes)